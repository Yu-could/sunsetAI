<template>
  <div class="px-4 py-4 w-full max-w-lg mx-auto pb-24">
      ...
      <div class="bg-white/10 rounded-xl p-3">
        <div class="flex items-center justify-between mb-2">
          ...
          <button @click="toggleWeatherExpand" class="text-white/80 hover:text-white text-xs flex items-center gap-1 flex-shrink-0 ml-2">
            <span class="hidden sm:inline">{{ isWeatherExpanded ? '收起' : '详情' }}</span>
        ...
        <div v-if="isWeatherExpanded" class="space-y-2 mt-3">
          ...
          <div class="bg-white/10 rounded-lg p-2">
            <p class="text-xs font-medium mb-1.5">📅 日程联动建议</p>
            <div class="space-y-1">
              <div v-for="(schedule, index) in weatherScheduleSuggestions" :key="index" class="flex items-center gap-1.5 text-xs">
                <span>{{ schedule.icon }}</span>
                <span class="text-white/90">{{ schedule.time }}</span>
                <span class="text-white/80">{{ schedule.activity }}</span>
                <span :class="schedule.recommended ? 'text-green-300' : 'text-amber-300'" class="ml-auto">
        ...
        <div class="flex justify-around mt-2 pt-2 border-t border-white/10">
          <div v-for="(day, index) in weatherData.forecast" :key="day.day" class="text-center px-1.5">
    ...
    <div class="bg-gradient-to-br from-parent to-parent-hover rounded-xl p-4 text-white shadow-md">
      ...
      <div v-if="showRecommendations.length > 0" class="mt-3 bg-white/20 rounded-lg p-2">
        <p class="text-xs text-white/80 mb-2">💡 您是不是想说：</p>
        <div class="flex flex-wrap gap-1.5">
          <button 
            v-for="rec in showRecommendations" 
            :key="rec.command"
            @click="handleRecommendClick(rec.route)"
            class="px-2 py-1 bg-white/30 hover:bg-white/50 rounded-full text-xs text-white transition-colors"
          >
            {{ rec.command }}
          </button>
        </div>
...
const weatherData = ref({
  ...
  forecast: [
    { day: '今天', date: '6/21', icon: '☀️', high: 30, low: 22 },
    { day: '明天', date: '6/22', icon: '⛅', high: 29, low: 21 },
    { day: '后天', date: '6/23', icon: '🌧️', high: 25, low: 19 },
    { day: '周五', date: '6/24', icon: '☁️', high: 27, low: 20 },
    { day: '周六', date: '6/25', icon: '☀️', high: 31, low: 23 }
  ],
...
const handleRecommendClick = (route) => {
  ...
  const response = responses[route] || '好的，为您打开相关页面'
...
const handleVoiceResult = (text) => {
  ...
  if (match.recommendations && match.recommendations.length > 0) {
    isSpeaking.value = true
    const suggestions = match.recommendations.map(r => r.suggestion).join(' ')
    voiceAssistant.speak(`抱歉，我没有听懂。${suggestions}`)
    showRecommendations.value = match.recommendations
    setTimeout(() => {
      isSpeaking.value = false
    }, 3000)
  } else {

  ...
  <div class="p-8 max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="text-gray-500 mt-1">记录生活点滴，珍藏人生故事</p>
      </div>
      <div v-if="activeTab === 'memoir'" class="flex items-center gap-2">
        <button @click="startRecording" 
          :class="isRecording ? 'bg-red-500 animate-pulse' : 'bg-orange-500'"
          class="flex items-center gap-2 px-6 py-3 text-white rounded-lg hover:opacity-90 transition-colors">
          <span class="text-xl">{{ isRecording ? '⏹️' : '🎤' }}</span>
          <span>{{ isRecording ? '停止录音' : '语音录入' }}</span>
        </button>
      </div>
      <div v-else-if="activeTab === 'photos'">
        <button @click="openPhotoUpload" class="flex items-center gap-2 px-6 py-3 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors">
          <span class="text-xl">📤</span>
          <span>上传照片</span>
        </button>
      </div>
      <div v-else>
        <button @click="openDiaryModal" class="flex items-center gap-2 px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm p-1 mb-6 inline-flex">
      <button @click="activeTab = 'diary'" 
        ...
        class="px-6 py-3 rounded-lg text-sm font-medium transition-all flex items-center gap-2">
        <span>📔</span> 日记
      </button>
    ...
    <!-- 相册模块 -->
    <div v-if="activeTab === 'photos'" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
          ...
          <div v-if="photoList.length > 0" class="grid grid-cols-4 gap-3">
      ...
      <div class="space-y-6">
        <div class="bg-gradient-to-r from-purple-500 to-indigo-500 rounded-xl p-6 text-white">
          <h3 class="text-lg font-bold mb-3">🖼️ 我的相册</h3>
          <p class="text-purple-200 text-sm mb-4">珍藏美好瞬间，AI帮您修复旧照片</p>
          <button @click="openPhotoUpload" class="w-full bg-white text-purple-600 py-3 rounded-lg font-medium hover:bg-purple-50">
            上传照片
          </button>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
          ...
          <div class="grid grid-cols-2 gap-3">
            <button @click="openDiaryModal" class="flex flex-col items-center justify-center p-3 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">
              <span class="text-2xl mb-1">✏️</span>
              <span class="text-xs font-medium">写日记</span>
            </button>
            <button @click="openPhotoUpload" class="flex flex-col items-center justify-center p-3 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors">
    ...
    <div v-if="activeTab === 'diary'" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2">
        <div class="bg-white rounded-xl shadow-sm p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-bold text-gray-800">📔 我的日记</h3>
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm text-gray-400">{{ formatDate(diary.date) }}</span>
                  </div>
                  <p class="text-gray-600 mt-2 line-clamp-2">{{ diary.content }}</p>
                  <div class="flex gap-2 mt-3">
                    <button @click="viewDiary(diary)" class="text-sm text-blue-500 hover:text-blue-700">👁️ 查看</button>
                    <button @click="editDiary(diary, index)" class="text-sm text-gray-500 hover:text-gray-700">✏️ 编辑</button>
                    <button @click="deleteDiary(index)" class="text-sm text-red-500 hover:text-red-700">🗑️ 删除</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div class="space-y-6">
        <div class="bg-white rounded-xl shadow-sm p-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">📊 本月心情</h3>
        ...
        <div class="bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl p-6 text-white">
          <h3 class="text-lg font-bold mb-3">✏️ 今日记录</h3>
          <p class="text-blue-200 text-sm mb-4">记录每一天的点滴，留住美好时光</p>
          <button @click="openDiaryModal" class="w-full bg-white text-blue-600 py-3 rounded-lg font-medium hover:bg-blue-50">
            写日记
          </button>
        </div>
      </div>
    ...
    <div v-if="activeTab === 'memoir'" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2">
        <div v-if="isRecording" class="bg-gradient-to-r from-orange-500 to-amber-500 rounded-xl p-6 text-white mb-6">
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center animate-pulse">
              <span class="text-3xl">🎤</span>
            </div>
            <div>
              <h3 class="text-xl font-bold">正在录制...</h3>
              <p class="text-orange-200 mt-1">请讲述您的故事，我正在聆听...</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-bold text-gray-700">📖 我的回忆录</h3>
          ...
          <div v-if="memoirList.length > 0" class="space-y-4">
            <div v-for="(memoir, index) in memoirList" :key="index" 
              ...
              <div class="flex items-start justify-between mb-2">
                ...
                <span v-if="memoir.aiPolished" class="bg-purple-100 text-purple-600 text-xs px-2 py-1 rounded-full">✨ AI润色</span>
              </div>
              <h4 class="font-bold text-gray-800 mb-2">{{ memoir.title }}</h4>
              <p class="text-gray-600 line-clamp-3">{{ memoir.content }}</p>
              <div class="flex gap-2 mt-3">
                <button @click="viewMemoir(memoir)" class="bg-blue-50 text-blue-600 px-3 py-1 rounded-full text-sm hover:bg-blue-100">阅读全文</button>
                <button @click="polishWithAI(memoir, index)" class="bg-purple-50 text-purple-600 px-3 py-1 rounded-full text-sm hover:bg-purple-100">✨ AI润色</button>
                <button @click="editMemoir(memoir, index)" class="bg-orange-50 text-orange-600 px-3 py-1 rounded-full text-sm hover:bg-orange-100">编辑</button>
                <button @click="deleteMemoir(index)" class="text-gray-400 hover:text-red-500 px-2 py-1">🗑️</button>
              </div>
            </div>
          </div>
          <div v-else class="text-center py-10 text-gray-400">
            ...
            <p class="text-sm">点击上方"语音录入"按钮记录您的故事</p>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-6">
        <div class="bg-white rounded-xl shadow-sm p-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">分类统计</h3>
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              <span class="text-gray-600">生活趣事</span>
              <span class="font-medium text-gray-800">{{ memoirList.length }}</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-2">
              <div class="bg-purple-500 h-2 rounded-full" :style="{ width: Math.min(memoirList.length * 10, 100) + '%' }"></div>
            </div>
          </div>
        </div>

        <div class="bg-white rounded-xl shadow-sm p-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">最近标签</h3>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <button @click="startRecording" class="flex flex-col items-center justify-center p-3 bg-orange-50 hover:bg-orange-100 rounded-lg transition-colors">
              <span class="text-2xl mb-1">🎤</span>
              <span class="text-xs font-medium">语音录入</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-bold text-gray-800">📖 我的回忆录</h3>
            <span class="text-sm text-gray-400">{{ memoirList.length }} 篇</span>
          </div>
        ...
        <div class="mb-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            <button v-for="style in polishStyles" :key="style.value"
              ...
              :class="selectedStyle === style.value ? 'bg-purple-100 border-purple-500' : 'bg-gray-50 border-gray-200'"
              class="p-3 rounded-lg border-2 text-center hover:border-purple-300 transition-colors">
              <span class="text-2xl">{{ style.icon }}</span>
...
const polishStyles = [
  { value: 'warm', label: '温馨风格', icon: '🌸', desc: '温暖亲切' },
  { value: 'formal', label: '正式风格', icon: '📜', desc: '庄重典雅' },
  { value: 'poetic', label: '诗意风格', icon: '🎨', desc: '优美抒情' },
  { value: 'concise', label: '简洁风格', icon: '✂️', desc: '简明扼要' }
]
...
const moodStats = computed(() => {
  ...
  return moods.map(m => ({ ...m, count: stats[m.value] }))
})
...
const openDiaryModal = () => {
  ...
  diaryForm.value = { title: '', content: '', mood: '😊', isPrivate: false }
